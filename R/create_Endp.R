...
create_Endp <- function (chemnames) {
                for (name in chemnames) {
                  ...
                  # Create matrix of endpoint names and their corresponding interval
                  name <- c("SUM_L", "SUM_D", "SUM_T", "AUC_L", "AUC_D", "AUC_T")
                  interval <- rep(list(names(`sheet`)[16:35], names(`sheet`)[36:55], names(`sheet`)[16:55]),
                                  2)
                  endpoints <- as.matrix(cbind(name, interval))
                  # Create SUM endpoints
                  invisible(apply(endpoints, 1, function(endp) {
                                                  `sheet`[, endp$`name`:=rowSums(.SD), .SDcols=endp$`interval`]}))
