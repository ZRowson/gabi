#' Hitcalling Function Editted by Zachary Rowson (Rowson.Zachary@epa.gov)
#' First Edit: 08/31/2021
#' Last Edit: 08/31/2021
#'
...
#'     \item cutoff.int - asymmetric noise cutoff interval about 0
...
#' @export
#'
tcplhit2_coreZR <- function(params, conc, resp, cutoff.int, onesd,bmr_scale = 1.349, bmed = 0, conthits = TRUE, aicc = FALSE, identifiers = NULL, bmd_low_bnd = NULL, bmd_up_bnd = NULL) {
  ...
  #------------------------------------------------------------------------------
  # Zachary Rowson Edit
  # Calculate cutoff based upon sign(top)
  if (sign(top) == 1) {cutoff <- abs(cutoff.int[[2]])} else {cutoff <- abs(cutoff.int[[1]])}
  #------------------------------------------------------------------------------
