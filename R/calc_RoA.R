#' Calculate Range of Activity per light level for individual fish
#'
#' @author Zachary Rowson \email{Rowson.Zachary@@epa.gov}
#'
#' @description
#' Calculates range of activity measurements per light period (light or dark).
#' User provides a lmr0 table and inputs
#' number of time periods in each experimental stage:
#' acclimation, light, and dark.
#'
#' @details RoA_L = max(speed in light) / min(speed in light), RoA_D = max(speed in dark) / min(speed in dark).
#' Created 09/28/2021. Last edit 11/15/2022.
#'
...
#'
#' @return A list with Range of Activity endpoints for each fish
#'   \itemize{
#'     \item RoA_L - speed ratio in light. max(speed) / min(speed)
#'     \item RoA_D - speed ratio in dark. max(speed) / min(speed)
#'   }
#' @export
calc_RoA <- function(data, no.A = 10, no.L = 20, no.D = 20) {

  rownames(data) <- NULL
  # find time-periods and sets corresponding to light and dark
  t <- grep("vt", names(data), value = TRUE)
  sets <- list(RoA_L = t[(no.A+1):(no.A+no.L)],
               RoA_D = t[(no.A+no.L+1):(no.A+no.L+no.D)])
  # calculate average speed in light and dark per fish
  RoA <- lapply(sets, function(set) {
    temp <- data[set]
